...
|----------|---------|---------------|-------------|
| 1 | Multi-route dashboard OpenAPI (establishment / visits style) | `api-gateway/02-...` | `prd/dish_pos_openapi_spec.yml` / `dev/swagger_api_gateway.yaml` / panel dashboard YAML |
| 2 | Apigee proxy / product / KVM pattern (placeholders only) | `apigee/01-...` | Optional local `Documents/API` notes if present |
| 3 | API key creation + referrer/IP/API restriction template | `api-and-services-keys/01-...` | Platform practice; no real key material |
